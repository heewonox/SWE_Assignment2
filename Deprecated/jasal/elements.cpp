...
void IDList::addNewAccount(string id,string password,string phone){
	...
	list[idx++]=newAccount;
	//tmp
	cout<<"inside IDList "<<newAccount->getID()<<" "<<newAccount->getPassword()<<" "<<newAccount->getPhone()<<endl;
}
...
void BicycleList::addNewBicycle(string id,string productName){
	...
	//tmp
	cout<<"inside BicycleList "<<newBicycle->getID()<<" "<<newBicycle->getProductName()<<endl;
}
